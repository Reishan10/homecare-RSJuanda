...
@section('content')
                            ...
                            <div class="tab-pane show active" id="basic-preview">
                                <div class="d-md-flex justify-content-between">
                                    <div class="d-flex align-items-center mb-3">
                                        <a href="{{ route('fisioterapi.printPDF') }}" class="btn btn-secondary btn-sm"
                                            id="btnExportPDF"> <i class="mdi mdi-file-pdf"></i> Export PDF</a>
                                        <a href="{{ route('fisioterapi.exportExcel') }}"
    ...
    <!-- fisioterapi modal -->
    <div id="fisioterapiModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="fisioterapiModalLabel"
        aria-hidden="true">
            ...
            <div class="modal-content">
                <form id="form">
                    ...
                    <div class="modal-header">
                        <h4 class="modal-title" id="fisioterapiModalLabel"></h4>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
                    </div>
                    <div class="modal-body">
                        <div class="mb-3">
                            <input type="hidden" name="id" id="id">
                            <label for="name" class="form-label">Fisioterapi</label>
    ...
    <div class="modal fade" id="detailModal" tabindex="-1" aria-labelledby="detailModalLabel" aria-hidden="true">
        <div class="modal-dialog  modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="detailModalLabel">Detail Fisioterapi</h5>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('body').on('click', '#btn-detail', function() {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ url('/fisioterapi/detail/"+id+"') }}",
                    data: {
                        id: id
                    },
                    dataType: 'json',
                    success: function(response) {
                        $('#name_detail').text(response.fisioterapi.name);
                        $('#deskripsi_detail').text(response.fisioterapi.deskripsi);
                        $('#harga_detail').text(response.fisioterapi.harga);
                    },
            ...
            $('#datatable').DataTable({
                ...
                columns: [{
                    ...
                    {
                        data: 'kode_fisioterapi',
                        name: 'kode_fisioterapi'
                    },
                    {
                        data: 'name',
                        name: 'Nama'
            ...
            $('#btnPrint').on('click', function() {
                ...
                var printContent =
                    '<table class="table"><thead><tr><th>No</th><th>Kode Fisioterapi</th><th>Nama</th><th>Harga</th></tr></thead><tbody>';

                $.each(data, function(index, value) {
                    printContent += '<tr><td>' + (index + 1) + '</td><td>' + value
                        .kode_fisioterapi + '</td><td>' + value.name + '</td><td>' + value.harga +
                        '</td></tr>';
                });
                ...
                printWindow.document.write('<html><head><title>Print Fisioterapi</title>');
            ...
            $('#btnTambah').click(function() {
                $('#id').val('');
                $('#fisioterapiModalLabel').html("Tambah fisioterapi");
                $('#fisioterapiModal').modal('show');
                $('#form').trigger("reset");
            ...
            $('body').on('click', '#btnEdit', function() {
                ...
                $.ajax({
                    type: "GET",
                    url: "fisioterapi/" + id + "/edit",
                    dataType: "json",
                    success: function(response) {
                        $('#fisioterapiModalLabel').html("Edit Fisioterapi");
                        $('#simpan').val("edit-fisioterapi");
                        $('#fisioterapiModal').modal('show');
            ...
            $('#btnHapusBanyak').on('click', function(e) {
                ...
                if (idArr.length <= 0) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: "{{ route('delete-multiple-fisioterapi') }}",
                                type: 'POST',
                                data: 'id=' + strId,
                                success: function(response) {
